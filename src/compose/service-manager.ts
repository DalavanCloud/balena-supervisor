...
export class ServiceManager extends (EventEmitter as {
	...
	public async start(service: Service) {
		...
		try {
			...
			await Bluebird.resolve(container.start())
				.catch(e => {
					...
					if (
						...
						this.config.get('deviceType').then(deviceType => {
							throw new Error(
				...
				.tapCatch(e => {
					...
					return Bluebird.resolve(container.remove({ v: true }))
						.catchReturn(_.noop)
						.then(() =>
		...
		const wait = (): Bluebird<void> =>
			Bluebird.any(
				...
				if (Date.now() < deadline) {
					return Bluebird.delay(pollInterval).then(wait);
				} else {
					console.log(
